// Vitya_1.cpp: определяет точку входа для консольного приложения.
//
...
// Формирование и выбор пункта меню
int getMenuItem()
{
	int item_menu = -1;
...
// Сумма четных и произведение нечетных
...
// Вывод элементов больше среднего арифметического элементов
...
// Добавления после элементов, равных введенному числу, суммы элементов, предшествующих ему
int insSumAfterValue(int *arrWork, int cnt)
{
	...
	for (int i = 0; i < cnt; i++)
	{
		...
		{
			// Дополниетельный элемент (сумма элементов до этого элемента)
...
// Обмен в массиве максимального элемента среди четных и минимального элемента среди нечетных
int changeMinMax(int *arrWork, int cnt)
{
	...
	// Проверка на размерность массива >= 2
	if (cnt == 1)
	{
	...
	// начальные значения для Min и Max
...
int changeMax3(int *arrWork, int cnt)
{
	...
	// Определение значений Max для чисел, делящихся на 3 без остатка и суммы чисел с последней цифрой 3
	for (int i = 0; i < cnt; i++)
	{
...
// упорядочивания элементов, расположенных между минимальным и максимальным элементами массива
int sortMinMax(int *arrWork, int cnt)
{
	...
	// Сортировка между Min и Max значениями
	if (minIdx > maxIdx)	
	{
		maxValue = maxIdx;
		maxIdx = minIdx;
